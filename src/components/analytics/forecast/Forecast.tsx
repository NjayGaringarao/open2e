...
export const Forecast: React.FC<ForecastProps> = ({ data, className = "" }) => {
  const series = useMemo(() => {
    const sorted = [...data].sort(
      (a, b) => new Date(a.date).getTime() - new Date(b.date).getTime()
    );
    const base = sorted.map((d, i) => ({
      x: i,
      y: d.averageScore,
      date: d.date,
    }));
    ...
    const regression = base.map((p) => ({
      date: p.date,
      averageScore: p.y,
      trend: a + b * p.x,
    }));
    const forecast = Array.from({ length: Math.max(0, forecastHorizon) }).map(
      (_, k) => {
        const x = lastIndex + 1 + k;
        const y = a + b * x;
        const nextDate = new Date(
          sorted[sorted.length - 1]?.date || Date.now()
        );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        <div className="p-4 rounded-lg border border-uGrayLight/30">
          ...
          <p className="text-lg font-semibold text-primary">
            {series.length >= 2 &&
            series[series.length - 1].trend > series[0].trend
              ? "Upward"
              : "Downward"}
          </p>
        </div>
        <div className="p-4 rounded-lg border border-uGrayLight/30">
          ...
          <p className="text-lg font-semibold text-uBlue">
            {series.length
              ? (series[series.length - 1].trend as number).toFixed(2)
              : "-"}
          </p>
        </div>
        <div className="p-4 rounded-lg border border-uGrayLight/30">
