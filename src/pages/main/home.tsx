                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl">
                  {quickActions.map((action, index) => (
                    <Link key={index} to={action.path}>
                      <div className={`group relative bg-gradient-to-br ${action.bgGradient} rounded-2xl p-8 border border-transparent hover:border-opacity-50 transition-all duration-500 hover:scale-105 cursor-pointer h-full backdrop-blur-sm shadow-xl hover:shadow-2xl`}>
                        {/* Gradient Border Effect */}
                        <div className={`absolute inset-0 bg-gradient-to-br ${action.borderGradient} rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 -z-10`} />
                        ...
                        <h3 className="text-xl font-bold text-uGray mb-3">{action.title}</h3>
                        <p className="text-uGrayLight leading-relaxed">{action.description}</p>
                      </div>
                    </Link>
                  ))}
                </div>
